...
const Content = ({item}) => {
    const columnsPerson: any[] = [
        <TableBuilderColumn key='col_person_name' id="name" header="Navn">
            {(row) => <Link key={`link_${row.data.id}`} href={teamViewerLink(row)}>{row.data.name}</Link>}
        </TableBuilderColumn>,
    ...
    const columnsTeam: any[] = [
        ...
        <TableBuilderColumn key='col_team_description' id="description" header="Beskrivelse">
    ...
    const Head = () => (
        <Block>{item['description_productarea']}</Block>
    );
    ...
    const Tables = () => {
        if (item) {
            return (
                <Block width="100%">
                    <Block><LabelMedium>Ansatte</LabelMedium>{getPeople()}</Block>
